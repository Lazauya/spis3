...
{
public:
	BSExtraData() : next(nullptr) {}; //fudging a constructor, might work?
	virtual ~BSExtraData() = default; //using default, because the vtable is replaced at runtime anyway

	virtual UInt32 GetType(void) const = 0;
	virtual bool IsNotEqual(const BSExtraData *rhs) const { return false; }; //i'm just doing what himika told me to do!
